...
public class AlarmInfoProvider {
    public String query(AlarmInfoModel alarmInfoModel) {
        return new SQL() {
            {SELECT("ai.id id"
                ...
                WHERE_LIKE_ALL("ai.alarm_content",alarmInfoModel.getAlarmContent());
                if(alarmInfoModel.getAlarmTime() != null){
                    SimpleDateFormat simpleDateFormat=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                    String time = simpleDateFormat.format(alarmInfoModel.getAlarmTime());
                    WHERE_EQUAL_TO("Date(ai.alarm_time)", time);
                }
                WHERE_EQUAL_TO("ai.alarm_source", alarmInfoModel.getAlarmSource());
                WHERE_EQUAL_TO("ai.id", alarmInfoModel.getId());
                WHERE_LIKE_ALL("ju.job_user_name",alarmInfoModel.getJobSupervisorName() );
